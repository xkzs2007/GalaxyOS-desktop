...
class SynapseGATEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        from services.gat_layer import GAT
    ...
    def forward(self, graph: SynapseGraph) -> torch.Tensor:
        ...
        if graph.num_edges > 0:
            # 使用稀疏 edge_index，O(E) 内存，不构建稠密矩阵
            edge_index = graph.edge_index  # (2, E)
        else:
            # 无边时用自环
            edge_index = torch.stack([torch.arange(n, device=features.device)] * 2, dim=0)
...
class SynapseGraphSAGEEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        from services.graphsage_layer import GraphSAGE
