...
class ClawWorker:
    ...
    # ════════════════════════════════════════════════════════
    # MemGAS-SkVM: SkillCompiler + AssetRegistry UDS methods
    # ════════════════════════════════════════════════════════

    def compile_skill(self, p: dict) -> dict:
        """
        编译技能：SkillCompiler.compile() + AssetRegistry.register()
        
        Args:
            p: {"skill_text": "...", "skill_name": "...",
                "profile": {...} (可选), "harness": {...} (可选)}
        Returns:
            {"artifact": {...}, "asset_id": "...", "success": bool}
        """
        skill_text = p.get("skill_text", "")
        skill_name = p.get("skill_name", "")
        if not skill_text:
            return {"success": False, "error": "missing skill_text"}
        try:
            from skill_compiler import compile_skill as _cs
            ...
            asset = create_skill_asset(
                skill_id=f"skill_compiled_{skill_name}_{int(time.time())}",
                raw_content=skill_text,
                capability_profile=artifact.profile_footprint if hasattr(artifact, 'profile_footprint') else {},
                tags=[skill_name.split()[0] if skill_name else "compiled"],
                category="compiled_skill",
            )
            asset.compiled_artifact = artifact.to_dict() if hasattr(artifact, 'to_dict') else {}
            reg.register(asset)
            
            return {
                "success": True,
                "artifact": artifact.to_dict() if hasattr(artifact, 'to_dict') else {"optimized_text": artifact.optimized_text},
                "asset_id": asset.asset_id,
                "compilation_time_ms": artifact.compilation_time_ms if hasattr(artifact, 'compilation_time_ms') else 0,
                "warnings": artifact.warnings if hasattr(artifact, 'warnings') else [],
            }
        except Exception as e:
            import traceback
            return {"success": False, "error": str(e), "traceback": traceback.format_exc()[-600:]}

    def asset_search(self, p: dict) -> dict:
        """
        查询 AssetRegistry：按 capability / tag / category / text
        
        Args:
            p: {"query": "...", "capability_key": "...",
                "tag": "...", "category": "...", "top_k": 10}
        Returns:
            {"assets": [...], "count": int}
        """
        try:
            ...
            if query:
                assets = reg.search(query, top_k=top_k)
            elif capability_key:
                assets = reg.query_by_capability(capability_key)
            elif tag:
                assets = reg.query_by_tag(tag)
            elif category:
                assets = reg.query_by_category(category)
            else:
                assets = [reg.get(aid) for aid in reg.list_ids()[:top_k] if reg.get(aid)]
            
            # 序列化
            for a in assets[:top_k]:
                results.append({
                    "asset_id": a.asset_id,
                    "asset_type": a.asset_type.value,
                    "raw_content": a.raw_content[:500],
                    ...
                    "compiled": bool(a.compiled_artifact),
                })
    ...
    def asset_register(self, p: dict) -> dict:
        """
        注册自定义 KnowledgeAsset
        
        Args:
            p: {"asset_id": "...", "asset_type": "skill|memory|plugin|knowledge",
                "raw_content": "...", "tags": [...], "category": "..."}
        Returns:
            {"asset_id": "...", "success": bool}
        """
        try:
            from knowledge_asset import get_asset_registry, KnowledgeAsset, AssetType
            reg = get_asset_registry()
            
            asset = KnowledgeAsset(
                asset_id=p.get("asset_id", f"asset_{int(time.time())}"),
                asset_type=AssetType(p.get("asset_type", "memory")),
                raw_content=p.get("raw_content", ""),
                tags=p.get("tags", []),
                category=p.get("category", ""),
                source=p.get("source", "uds_register"),
                capability_profile=p.get("capability_profile", {}),
            )
            if p.get("compiled_artifact"):
                from skill_compiler import CompiledArtifact
                asset.compiled_artifact = p["compiled_artifact"]
            
            reg.register(asset)
            return {"asset_id": asset.asset_id, "success": True}
        except Exception as e:
            return {"success": False, "error": str(e)}
...
def _init_methods(worker):
    ...
    _METHODS = {
        ...
        # MemGAS-SkVM
        "compile_skill": worker.compile_skill,
        "asset_search": worker.asset_search,
        "asset_register": worker.asset_register,
    }
