...
_OPENCLAW_HOME = path_resolver.OPENCLAW_HOME  # 内部工具，保留默认路径
...
CONFIG_PATH = Path(os.environ.get("OPENCLAW_CONFIG_FILE", str(_OPENCLAW_HOME / "openclaw.json")))
    ...
    def test_embedding_latency(self):
        ...
        try:
            response = requests.post(
                f"{emb_config.get('baseUrl', 'https://cloud.infini-ai.com/maas/v1')}/embeddings",
                headers={
                    "Authorization": f"Bearer {emb_config['apiKey']}",
                    "Content-Type": "application/json"
                },
                json={
                    "model": emb_config.get("model", "bge-m3"),
                    "input": "测试文本",
