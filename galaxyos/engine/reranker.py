...
# ── Reranker 配置（无问芯�?bge-reranker-v2-m3，免费）──────────
RERANKER_URL = os.environ.get(
    "YAOYAO_RERANKER_URL",
    "https://cloud.infini-ai.com/maas/v1/rerank",
)

# �?unified_config.json �?embedding API key（与 embedding.py/llm_client.py 统一�?
...
def rerank_results(query: str, candidates: List[Dict], top_k: int = 10) -> List[Dict]:
    """
    �?bge-reranker-v2-m3 对候选结果重排序（公开接口�?

    query + documents 一起传，reranker �?cross-encoder 重新打分�?
    
    Args:
        query: 原始查询
        candidates: 候选结果列表（通常�?top_k * 2�?
        top_k: 返回结果�?
...
def __main():
    """CLI 测试入口"""
    print("测试用例 - 请从 Python 代码中调�?rerank_results()")
