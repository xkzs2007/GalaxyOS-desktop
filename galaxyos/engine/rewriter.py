...
class QueryRewriter:
    ...
    @classmethod
    def rewrite(cls, query: str, context: list = None) -> Tuple[str, List[str]]:
        """
        改写查询

        Args:
            query: 原始查询
            context: 可选的上下文列表（用于增强改写）

        Returns:
            Tuple[str, List[str]]: (改写后的查询, 修正列表)
        """
        _original = query
        corrections = []
    ...
    def expand(cls, query: str) -> List[str]:
        """
        扩展查询（别名方法，兼容旧代码）
        等同于 get_synonym_expansions()
        """
        return cls.get_synonym_expansions(query)
