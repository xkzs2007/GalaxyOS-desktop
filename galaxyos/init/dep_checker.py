    ...
    def print_report(self):
        ...
        for cat in ['core', 'recommended', 'optional']:
            ...
            for dep in deps:
                ...
                logger.info(f"  {status} [{label}] {dep.name}{ver} - {dep.description}")
                if not dep.installed:
                    logger.info(f"      💡 {dep.install_hint}")
        ...
        logger.info(f"  可用: {len(available)}/{len(self._module_results)}")
        ...
        if unavailable:
            ...
            for mod in unavailable:
                ...
                logger.info(f"    ❌ {mod.name} ({mod.category}) - 缺: {missing_str}")
        ...
        if total_missing > 0:
            ...
            if all_cmds:
                logger.info("")
                logger.info("  一键安装全部缺失依赖:")
                logger.info("    pip install %s", ' '.join(d.pip_name for d in self._results.values() if not d.installed and d.pip_name))
            ...
            if platform.system() == 'Linux':
                ...
                if apt_pkgs:
                    logger.info()
                    logger.info("  或使用系统包管理器:")
                    logger.info(f"    sudo apt install {' '.join(apt_pkgs)}")
        else:
            logger.info("🎉 所有依赖已安装，全部功能可用！")
