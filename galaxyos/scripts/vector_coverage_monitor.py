...
LOG_FILE = path_resolver.MEMORY_TDAI_DIR / ".metadata" / "coverage_monitor.log"
CONFIG_FILE = path_resolver.LLM_MEMORY_DIR / "config" / "coverage_thresholds.json"
...
class CoverageMonitor:
    ...
    def auto_fix(self, coverage_data):
        ...
        if coverage_data["l1_coverage"] < self.config["l1_min_coverage"]:
            ...
            try:
                result = subprocess.run(
                    ["python3", str(path_resolver.LLM_MEMORY_SCRIPTS / "backfill_l0_vectors.py"), "--l1"],
        ...
        if coverage_data["l0_coverage"] < self.config["l0_min_coverage"]:
            ...
            try:
                result = subprocess.run(
                    ["python3", str(path_resolver.LLM_MEMORY_SCRIPTS / "backfill_l0_vectors.py")],
