...
function resolveClawArgs(opts) {
  if (opts.lastDays !== undefined) {
    const n = parseInt(opts.lastDays, 10);
    if (isNaN(n) || n <= 0) {
      console.error("Error: --last-days must be a positive integer");
      process.exit(1);
    }
    const end = new Date;
...
async function callCeliaClaw(tools, skillId, verbose = false) {
  ...
  const randPart = String(randomInt(0, 1e6)).padStart(6, "0");
  const traceId = `${uid}-${datePart}-${randPart}`;
...
function registerClawCommands(program2) {
  for (const toolName of HEALTH_TOOLS) {
    program2.command(toolName).description(`Fetch ${toolName} data via celia-claw`).option("--date <date>", 'Single date (YYYY-MM-DD or "today")').option("--start-date <date>", "Range start date (YYYY-MM-DD)").option("--end-date <date>", "Range end date (YYYY-MM-DD)").option("--last-days <n>", "Query the last N days (takes priority over other date options)").option("--skill-id <id>", "Value sent in x-skill-id header", "xiaoyi_health").option("--verbose", "Print loaded env vars and request details for debugging").action(async (opts) => {
      const args = resolveClawArgs(opts);
      await callCeliaClaw([{ name: toolName, args }], opts.skillId, opts.verbose ?? false);
    });
  }
}
...
program2.name("pha-claw").description(`Execute PHA health data tools via the celia-claw gateway.

` + "Required env vars: SERVICE_URL, PERSONAL_UID, PERSONAL_API_KEY").version("0.1.0").option("--tools <json>", 'JSON array of tool calls: [{"name":"...","args":{...}}]').option("--skill-id <id>", "Value sent in x-skill-id header", "xiaoyi_health").option("--verbose", "Print loaded env vars and request details for debugging").action(async (opts) => {
  if (!opts.tools) {
    program2.help();
    return;
  }
  ...
  try {
    ...
    console.error("Error: --tools value must be valid JSON");
    process.exit(1);
  }
  await callCeliaClaw(tools, opts.skillId, opts.verbose ?? false);
});
registerClawCommands(program2);
program2.parse();
