...
export default function register(api) {
    ...
    const _lastAssembleDecisions = new Map();  // sessionId → {isrel, crag, coevolve, skill0, ...}
    ...
    api.registerContextEngine("claw-core-engine", (ctx) => ({
            ...
            async assemble({ sessionId, messages, tokenBudget, availableTools, citationsMode }) {
                ...
                try {
                    ...
                    // ────────────────────────────────────────────
                    // 4) decisions 协议 — Worker 产出的结构化决策
                    //
                    // AssembleDecisions {
                    //   isrel?:   { should_retrieve: boolean, confidence: number }
                    //   crag?:    { action: "USE"|"DISCARD"|"AUGMENT", quality: number }
                    //   coevolve?:{ failure_detected: boolean, pattern: string, action: string }
                    //   skill0?:  { done: boolean, stage: number, active_count: number }
                    //   hyper_route?: { strategy: string }
                    // }
                    //
                    // applyDecisions() 将所有 decisions 行为判断集中到一处
                    // ────────────────────────────────────────────
                    let _assembleDecs = {};  // decisions 供下游使用
                    let _ceCfg = {};  // applyDecisions 返回的策略配置

                    function applyDecisions(decisions) {
                        // 所有 decisions 驱动的行为控制集中在此
                        const cfg = { recallBudget: 600, skipAll: false, skipCrossSession: false, personaShort: false };
                        if (!decisions || Object.keys(decisions).length === 0) return cfg;

                        // isrel — 是否跳过全部检索
                        const isrel = decisions.isrel || {};
                        if (isrel.should_retrieve === false) {
                            cfg.skipAll = true;
                            cfg.recallBudget = 0;
                            return cfg;
                        }
                        ...
                        // crag — 检索质量驱动
                        const crag = decisions.crag || {};
                        const cragAction = (crag.action || "").toUpperCase();
                        if (cragAction === "DISCARD" || cragAction === "DISCARDED") {
                            cfg.skipCrossSession = true;
                        }
                        if (crag.quality != null && crag.quality < 0.4) {
                            cfg.recallBudget = Math.round(cfg.recallBudget * 0.5);
                        }

                        // coevolve — 失败模式
                        const coevolve = decisions.coevolve || {};
                        if (coevolve.failure_detected === true) {
                            api.logger.info?.(`${TAG} [context-engine] CoEvolve failed: ${JSON.stringify(coevolve)}`);
                        }

                        // skill0 — 技能内化
                        const skill0 = decisions.skill0 || {};
                        if (skill0.done === true) {
                            cfg.personaShort = true;
                        }
                        ...
                        try {
                            ...
                            if (lastUserMsg) {
                                ...
                                if (query && query.trim().length >= 5) {
                                    const w = getWorker(ws);
                                    if (w && w.ready) {
                                        // step 1: quick 模式预检 — 只跑 IsREL（几百微秒）
                                        const quickResult = await w.call("context_assemble", {
                                            query, session_id: sessionId, top_k: 5, mode: "quick"
                                        }, 3000).catch(() => null);
                                        const quickDecs = quickResult?.decisions || {};
                                        _ceCfg = applyDecisions(quickDecs);
                                        ...
                                        if (!_ceCfg.skipAll) {
                                            ...
                                            try {
                                                const ctxResult = await w.call("context_assemble", {
                                                    query, session_id: sessionId, top_k: 5, mode: "full"
                                                }, 15000);
                                                ...
                                                // 存储完整 decisions 供行为决策
                                                _assembleDecs = ctxResult?.decisions || {};
                                                _lastAssembleDecisions.set(sessionId, _assembleDecs);
                                                // 覆盖 quick 预检的配置（全量 decisions 更准确）
                                                _ceCfg = applyDecisions(_assembleDecs);
                                            } catch (e) { /* 降级 */ }

                                            // 降级: smartRecall — 只在 IsREL 未禁止检索时兜底
                                            if (!injection && !_ceCfg.skipAll) {
                                                const isrel = _assembleDecs.isrel || quickDecs.isrel || {};
                                                if (isrel.should_retrieve !== false) {
                                                    ...
                                                    api.logger.debug?.(`${TAG} [context-engine] IsREL skip retrieve, no fallback`);
                                                }
                                            }
                                            systemPromptAddition = injection;
                                        }

                                        // step 3: 跨会话 DAG 搜索 — 仅当 CRAG 未判定 discard 时才执行
                                        if (CE_RECALL_ON_ASSEMBLE && !_ceCfg.skipCrossSession) {
                                            try {
                    ...
                    // ────────────────────────────────────────────
                    // 5) 人格注入 — 已在 4a 中由 applyDecisions 精简
                    //    实际注入在下方 personaBlock 构建中（已内聚到 4a）
                    // ────────────────────────────────────────────
