...
class LfmSkillBank:
    ...
    def save(self, filepath: Optional[str] = None) -> None:
        ...
        if not path:
            ws = self.config.workspace or os.environ.get("OPENCLAW_WORKSPACE", str(Path.home() / ".openclaw" / "workspace"))
...
def main():
    ...
    workspace = sys.argv[1] if len(sys.argv) > 1 else str(Path.home() / ".openclaw" / "workspace")
