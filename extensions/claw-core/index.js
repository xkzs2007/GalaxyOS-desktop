...
class WorkerPool {
    constructor(ws, cfg = {}) {
        ...
        this.workers = new Map();           // id → ClawWorkerClient
        this.busy = new Set();               // id of busy workers
        this.queue = [];                     // [{priority, run, resolve, reject, label, ts}]
        this.counter = 0;
    ...
    _init() {
        ...
        // 健康检查：30s 轮询
        this._healthTimer = setInterval(() => this._healthCheck(), 30000).unref();
        this._ready = true;
    }
    ...
    _spawnOne(id) {
        ...
        w.start().catch((e) => {
            process.stderr.write(`[claw-core] pool spawn ${id} failed: ${e.message}\n`);
        });
    ...
    // 主调度入口：执行一个 task，支持优先级排队
    async execute(method, params, priority = 'normal', timeoutMs = 30000) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (priority === 'high') {
                ...
                // normal：插到最后一个 normal 后面，保持 high 优先
    ...
    _healthCheck() {
        for (const [id, w] of this.workers) {
            if (!w.ready || w._fails >= w._maxFails) {
                process.stderr.write(`[claw-core] pool health: ${id} unhealthy, respawning...\n`);
                w.stop();
    ...
    async shutdown() {
        ...
        for (const [id, w] of this.workers) {
            promises.push(
                w.call('shutdown', {}, 2000).catch(() => {}).finally(() => w.stop())
            );
        }
...
function getWorker(ws) {
    ...
    if (!_workerPool) {
        try {
            ...
            process.stderr.write(`[claw-core] WorkerPool initialized (size=${poolCfg.size})\n`);
        } catch (e) {
            process.stderr.write(`[claw-core] WorkerPool init failed, fallback to single worker: ${e.message}\n`);
        }
    }
    // 池可用 → 返回调度包装
