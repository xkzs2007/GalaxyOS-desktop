...
@register_tool(
    name="rccam.process",
    description="R-CCAM 结构化认知循环 - 对用户输入执行完整五阶段循环：Retrieval → Cognition → Control → Action → Memory，返回结构化思考结果",
    category="core",
    parameters={
        "user_input": {"type": "string", "required": True, "description": "用户输入文本"},
        "max_cycles": {"type": "integer", "required": False, "default": 1, "description": "最大循环轮次（默认1，最多3）"},
        "store_memory": {"type": "boolean", "required": False, "default": True, "description": "是否持久化记忆"}
    },
    returns={"type": "object", "description": "包含回答、置信度、策略、分类等结构化结果"}
)
def rccam_process(user_input: str, max_cycles: int = 1, store_memory: bool = True) -> dict:
    """R-CCAM 五阶段认知循环

    对输入执行完整五阶段认知循环，返回结构化思考结果。
    依赖: xiaoyi_claw_api.XiaoYiClawLLM
    """
