...
> **更新时间**: 2026-05-14 23:15
> **架构层数**: 15 层（17层→15层精简）
> **总功能数**: 350+ 项
> **IPC 通道**: UDS RPC + ZMQ 事件推送 + mmap 共享内存
> **Lobster 管道**: 0（全部由 IPC RPC 替代）
...
1. **记忆能力** — 三层记忆体系（腾讯云插件 + yaoyao Memory + 本地记忆系统）+ 记忆巩固引擎（CLS固化 + 离线重放 + 艾宾浩斯遗忘曲线）
2. **检索能力** — 向量检索 + 知识图谱 + Self-RAG + CRAG 混合检索 + CRAG 动态纠错 + 场景锚定注入（Drawing on Memory 双迹编码 + GRAVITY 结构锚定）+ bge-reranker-v2-m3 重排序 + 预测编码冲突检测
3. **智能处理能力** — 查询改写（Pro）+ 结果总结（Flash）+ 语义过滤（SmartProcessor 双模型通道）+ 自进化上下文注入 + Flash 开推理场景编码 + KV 缓存优化 + Flash NLP 路由
4. **思考能力** — 29 技能全映射（9思考 + 11方法论 + 10工程）+ Reflexion 反思 + Self-Refine 精炼 + Multi-Path 多路径 + 增强 NLP（依存句法/实体链接/指代消解/对比检测/Flash API 路由）
5. **执行能力** — R-CCAM 结构化认知循环 + DAG 上下文中继 + Worker 三通道 IPC（UDS/ZMQ/mmap）+ 并行工作流引擎 + 自动重启
6. **多模态能力** — 图像理解 + 图像生成 + OCR2 深度整合 + Visual RAG
7. **可靠性能力** — 防幻觉 + 自我修复 + 故障转移 + ACP 持久化通道 + Rails 护栏 + 隐式偏好学习 + 硬件加速自动降级

---

| **文档版本**: v4.0 | 2026-05-14 23:15 | 三通道 IPC + 15 层精简 + 0 Lobster |

---

*小艺 Claw — OpenClaw 的核心底层能力引擎*
*文档版本: v3.4.1 | 最后更新: 2026-05-13*
...
└─────────────────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                      小艺 Claw 系统架构 v4.0 (15层 · 350+功能)             │
├─────────────────────────────────────────────────────────────────────────────┤
│  ┌─ Layer 0: 记忆架构 —————————————————————————————————────────────────┐   │
│  │  腾讯云记忆插件(L0对话+L1结构化+场景归纳) + Session State快照       │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 0.5: 云端备份 ————————————————————————————————————————┐   │
│  │  华为云盘(7天全量) + IMA知识库(24h增量)                        │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 1: 记忆核心层 (Memory Core) ★ IPC ——————————————————┐   │
│  │  ▸ 防幻觉: Self-RAG(2023)+CRAG(2024) · 10重检测             │   │
│  │  ▸ 突触LTP/LTD: Hebbian · 艾宾浩斯 R=e^(-t/S) [1885]       │   │
│  │  ▸ 记忆巩固: CLS(McClelland 1995)+Replay(Wilson 1994)       │   │
│  │  ▸ 干扰合并(Friston 2010)+情感记忆(Ekman 6)                 │   │
│  │  ▸ IPC: MemoryService独立线程池, MKL/AVX-512加速embedding    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 2: 检索增强层 (Retrieval) ★ IPC ——————————————————┐   │
│  │  ▸ Self-RAG(isrel/issup/isuse) + CRAG动态纠错              │   │
│  │  ▸ 混合检索: Dense(FAISS/Qdrant·bge-m3 1024维)+Sparse(RRF) │   │
│  │  ▸ 命题检索 + 交叉验证(记忆↔搜索↔KG)                       │   │
│  │  ▸ 重排序: bge-reranker-v2-m3 (无问芯穹)                   │   │
│  │  ▸ IPC: RetrievalService独立线程池, Dense+Sparse并行        │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 3: 向量存储层 ——————————————————————————————————┐   │
│  │  L1 sqlite-vec(C) → L2 hnswlib(O(log n)) →              │   │
│  │  L3 内置HNSW(Python) → L4 numpy暴力(O(n)) 四级降级       │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 4: 智能处理层 (SmartProcessor) ——————————————┐   │
│  │  快通道(Flash/简单查询) + 重通道(Pro/改写·总结·过滤)    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 5: LLM优化层 ————————————————————————————————┐   │
│  │  投机解码L1(检索Trie)+L2(NIM·40次/分)+L3(Flash验证)   │   │
│  │  KV Cache复用(X-Conversation-Id·缓存价2.5折) + ACP通道 │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 6-8: 缓存·硬件·可靠性 —————————————————————┐   │
│  │  ▸ 语义/KV/RAG缓存 + MKL/AVX-512/FMA3+NUMA绑定      │   │
│  │  ▸ 硬件自动降级: MKL→AVX-512→SSE4.2                  │   │
│  │  ▸ 故障转移·自动恢复·57弹性组件                      │   │
│  │  ▸ IPC: HardwareService独立线程池, mmap共享状态       │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 9: 会话管理层 (Session) ★ IPC ————————————┐   │
│  │  ▸ DAG上下文中继(ContextEngine·ingest/assemble/compact) │   │
│  │  ▸ 熔断保护·全局窗口压缩·跨8层联动(L9+L1+L6+L11)    │   │
│  │  ▸ IPC: SessionService独立线程池, UDS dag_ingest     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 10: Persona管理层 ——————————————————————————┐   │
│  │  7重防线: AGENTS.md·bootstrap·headChars扩容·校验    │   │
│  │  ·记忆恢复·快照·自进化固化                          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 11: NLP能力层 ★ IPC ————————————————————┐   │
│  │  ▸ 基础(0ms): jieba分词+NER+TF-IDF+SnowNLP+摘要   │   │
│  │  ▸ 增强(论文): 依存句法[Ramshaw 1995]+实体链接     │   │
│  │     [Shen 2015]+指代消解[Hobbs 1978]+对比检测      │   │
│  │     [Jindal 2006]                                 │   │
│  │  ▸ Flash路由: 指代消解·对比检测·意图分析(复杂语义)  │   │
│  │  ▸ IPC: ThinkingService独立线程池                  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 12: 思考技能层 ———————————————————————————┐   │
│  │  ▸ 9思考+11方法论+10工程 = 29技能全映射          │   │
│  │  ▸ Reflexion[Shinn 2023]+Self-Refine[Madaan 2023] │   │
│  │  ▸ Multi-Path[Yao 2023]+RAPTOR[Sarthi 2024]       │   │
│  │  ▸ GraphRAG[MS 2024]+GA反思[Park 2023]+Toolformer  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 13: 多模态层 ————————————————————————————┐   │
│  │  图像理解(双引擎)+生成(seedream)+OCR2           │   │
│  │  Visual RAG[Gupta 2024]: Cognition自动OCR2注入   │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 14: R-CCAM 认知循环层 (核心调度器) ★ ———————┐   │
│  │  R→C→C→A→M 五阶段循环 · 44工作流IPC并行调度         │   │
│  │  多Agent批评者+LLM-as-Judge+隐式偏好收集            │   │
│  │  0 Lobster(全删.全走IPC RPC)                       │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│  ┌─ Layer 15: 系统能力层 (三通道IPC) ★ ————————————┐   │
│  │  ▸ UDS RPC(/tmp/claw-worker.sock) 4字节+JSON     │   │
│  │  ▸ ZMQ PUB(tcp://127.0.0.1:5559) 事件推送        │   │
│  │  ▸ mmap(/dev/shm 10MB) TTL60s 零拷贝              │   │
│  │  ▸ IPC自动选路: <1KB→UDS/1-100KB→mmap/>100KB轮询  │   │
│  │  ▸ _rails.py护栏+Worker自动重启+自进化            │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                           ┌──────────┐                                    │
│                           │ 15层总计  │                                    │
│                           │ 350+功能  │                                    │
...
| 指标 | 数值 | 更新时间 |
|------|------|----------|
| 架构层数 | **15 层** | 2026-05-14 |
| 总功能数 | **350+ 项** | 2026-05-14 |
| IPC 三通道 | **3** (UDS+ZMQ+mmap) | 2026-05-14 |
| 独立服务层 | **5** (L1/L3/L6/L9/L11) | 2026-05-14 |
| Lobster 管道 | **0** (全删除·IPC替代) | 2026-05-14 |
...
### v4.0 (2026-05-14 20:10) — Visual RAG 自动 OCR2 路由
...
### v3.4.1 (2026-05-13 12:17) — 论文方向四扩展 + KV缓存全线修复 + Worker常驻批评者
...
*小艺 Claw — OpenClaw 的核心底层能力引擎*
*文档版本: v3.3.0 | 最后更新: 2026-05-12*

---

## 📝 更新日志

### v3.3.1 (2026-05-13) — 思考技能全联动 + DAG 上下文引擎修复 + scene_trace 回溯
- ✨ **思考技能全联动**：Cognition 阶段 29 个技能全映射（9思考 + 10MattPocock + 10方法论）
  - `SKILL_MAPPING` 扩展：Matt Pocock（diagnose/grill-with-docs/tdd/improve-arch/prototype/zoom-out/grill-me） + 方法论 11 项 + 效率 3 项（caveman/handoff/write-a-skill）
  - `MATT_POCOCK_SKILL_PATHS` + `THINKING_SKILL_PATHS` + `METHODOLOGY_SKILL_PATHS` + `ALL_SKILL_PATHS` 合并
  - 每技能自动读 SKILL.md 前 500 字注入 `state.analysis['skill_guide']`
  - 6 个新 question_type 模式：debug/architecture/review/code/prototype + reverse/compress/handoff/writeskill
- ✨ **R-CCAM Action 阶段技能注入**：`skill_guide` 拼进 Flash 整合/Pro 查询改写/Pro 结果总结的 system prompt
  - 不再通用"知识丰富的AI助手"，带方法论指导
- 🎯 **Worker dag_ingest 重写**：改用 `_ensure_smart_processor()` + httpx 直调生成 scene_trace
  - 绕开 DAGIntegration import 路径问题
  - 先 `dag.add_message()` 确保主流程成功，再异步生成 scene_trace
  - UPDATE SQLite 写入 scene_trace 列
- 🎯 **`_call_flash_with_thinking` 修复**：`content` + `reasoning_content` 双字段降级
  - DeepSeek V4 Flash thinking=True 时 content 可能空，靠 reasoning 补
  - SmartProcessor `_call_flash` 同修
- 🎯 **DAG Bug 修复**：`cross_session_memory_restore` 三处修复
  - `n.priority >= PriorityLevel.NORMAL`（原来 `<=` 全过滤掉）
  - `node.metadata` 是 JSON 字符串，需 `json.loads()` 后再取 role
  - Flash 调用失败时走纯文本 fallback（不再是空字符串）
- 🎯 **scene_trace 330 节点回溯填充**：`scripts/backfill_scene_trace.py` 脚本，268 + 62 个全成功
  - workspace DB 268/268 ✅ / old DB 62/62 ✅
- 🎯 **intelligent_thinking_trigger.py 整理**：SKILL_MAPPING 分 5 个区块（思考/方法论/Matt Pocock/效率/补充）
  - 原映射（first-principles/critical-thinking 等）因方法论插入被覆盖的已补回
  - `reverse`/`compare` 补充映射
- 📊 **统计数据更新**：技能数更新，修复 29 个技能缺映射的问题
- 📋 **架构文档 v3.3.1 重命名**：修复 v3.3.0 到 v3.3.1 版本升级 + 多处遗留的 v3.3.0 引用

### v3.3.0 (2026-05-12) — 三路记忆融合 + Merge Gate + Rails 护栏
- ✨ **三路记忆融合（Memory Unification）**：`unified_entry.py` 的 `store()` 和 `recall()` 打通 yaoyao DB 路，实现三路融合：
  - store() 三路写入：XiaoYiClawLLM.remember() 主路 → yaoyao DB（FTS5 + hnswlib 向量）→ XiaoyiMemoryV2 降级路
  - recall() 三路融合：XiaoYiClawLLM.recall() 主路 → yaoyao search_yaoyao() → RRF 融合 → XiaoyiMemoryV2 降级
  - 结果带 `source` 字段（`yaoyao`/`xiaoyi_claw`），方便调试
  - tencentdb_sync 工作流重写为真实同步（读 scene_blocks 目录逐个写入 yaoyao DB）
- ✨ **Merge Gate 合入门禁**：7 个测试场景全部通过
  - 5 项硬条件门禁 + Continuity Guard（HEAD SHA 校验）
  - 修复 2 个 Bug（condition 语法不兼容 + block 未用 exit code 1）
  - 通过/拦截逻辑完整验证，该过的过、该拦的拦
- ✨ **Rails 护栏挂载**：PermissionContext + @rail 装饰器挂载到 unified_entry.py 的 4 个关键方法（store/recall/health_check/workflow）
- ✨ **Yaoyao 插件定位修正**：架构图中 Yaoyao Memory 从 Layer 12（思考技能层）移至 L0 记忆架构层（与腾讯云插件、Session State 同级），Layer 12 仅保留思考技能和方法论技能
- ✨ **Layer 14 工作流 Lobster 管道新增**：merge-gate.lobster
- 📊 **统计数据更新**：总功能数 260+ → 270+ 项，新增三路融合 6 项、Merge Gate 8 项、Rails 护栏 4 项、架构修正 1 项
