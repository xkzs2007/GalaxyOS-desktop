...
5. **执行能力** — 44 个工作流全 IPC 并行调度 + R-CCAM 结构化认知循环（统一深度管线）+ DAG 上下文中继 + Worker UDS 主通道（Plugin直连，无stdin/stdout）+ ZMQ 事件推送 + mmap 共享内存 + Worker 自动重启 + **Merge Gate（五路检索去重合并，merged_context输出）** + 后台4论文引擎并行
...
7. **可靠性能力** — 防幻觉 10 重检测 + 自我修复 + 故障转移 + ACP 持久化通道 + 全局上下文窗口比例压缩 + 自进化决策执行层 + 系统消息噪声过滤 + **Rails 护栏增强版（检索结果扫敏感凭据/系统路径，认知分析扫违规建议）** + 隐式偏好学习 + Worker 自动重启 + Merge Gate 合入门禁 + **人格视觉（Persona Visual—DAG人格节点vs文件状态比对）** + **用户画像驱动内在元认知进化（Flash以工程严谨/论文驱动/全量推进/矛盾分析/第一性原理/系统思维/费曼技巧/批评与自我批评等视角分析体验数据，惰性激活AutoTuner/AutoPersonaUpdater/KnowledgeRefiner）**

---

| **文档版本**: v4.4 | 2026-05-20 18:06 | Merge Gate + Rails增强 + 人格视觉 + 用户画像驱动内在元认知 |

---

*小艺 Claw — OpenClaw 的核心底层能力引擎*
*文档版本: v4.4 | 最后更新: 2026-05-20 18:06*
...
| CNB 仓库 | **1323 个文件 / 312 MB** | 2026-05-19 |
...
| **事件驱动** | 备份、推送、同步均为事件触发 |
| **Merge Gate** | 五路检索去重合并，merged_context输出 |
| **人格视觉** | DAG人格节点vs文件状态比对，注入cognition_payload |
| **内在元认知** | 用户画像驱动，8种分析模式，惰性激活下游模块 |

---

## 📝 更新日志

### v4.4 (2026-05-20 18:06) — 人格视觉 + Merge Gate + Rails增强版 + 用户画像驱动内在元认知进化

- ✨ **人格视觉（Persona Visual）** — Layer 10 Persona 管理层新增组件
  - DAG 人格节点 vs 文件状态比对（存在性/时间戳/是否过期检测）
  - 检测不一致自动生成修复建议
  - 结果注入 cognition_payload（persona_health 字段）
- ✨ **Merge Gate（五路检索去重合并引擎）** — Layer 2 检索增强层 + Layer 14 Control 阶段
  - 输入: 向量检索 + BM25 + KG + DAG场景 + 联网搜索结果
  - 去重: embedding余弦相似度(阈值0.92)+文本Jaccard(阈值0.85)双判据
  - 排序: RRF融合分 + 来源信誉权重
  - 输出: merged_context — 统一上下文块供主模型
  - 元认知参数微调: 检索量高→knowledge_density升 / 来源多样→familiarity降
- ✨ **Rails 护栏增强版** — Layer 15 _rails.py
  - 检索结果扫描器: 敏感凭据(API Key/Token)/系统路径/私密数据
  - 认知分析扫描器: 违规建议检测(越狱/有害指令/社会工程)
- ✨ **用户画像驱动内在元认知进化** — Layer 16 自进化层（从"自进化上下文注入"升级）
  - Flash 以用户视角（8种分析模式）分析体验数据:
    工程严谨 | 论文驱动 | 全量推进 | 矛盾分析
    第一性原理 | 系统思维 | 费曼技巧 | 批评与自我批评
  - 结构化进化产出（问题描述/建议类型/优先级/预期效果）
  - 惰性激活下游模块（AutoTuner/AutoPersonaUpdater/KnowledgeRefiner）
- ✨ **__memory_phase 每10轮触发内在元认知分析**
- ✨ **__activate_callback** — 根据进化建议类型惰性加载下游模块
- 📊 **统计数据更新**：总功能数 360+ → 375+ 项，新增15+项

### v4.3 (2026-05-19 22:23) — CNB 仓库结构完善 + MIT 许可证 + 预编译库策略
- ✨ **CNB 仓库结构完善**：新增 README.md（完整仓库说明、安装方式、15层架构概览）
- ✨ **Python 包元数据**：新增 setup.py（`pip install -e .`，包名 `xiaoyiclaw==4.2.0`）、requirements.txt
- ✨ **预编译原生库**：libs/ 目录上传 MKL 核心库（91 MB）、TBB（4.3 MB）、hnswlib（224 KB）
- ✨ **部署脚本**：workspace-scripts/setup_libs.sh 一键解压预编译库
- ✨ **MIT 许可证**：新增 LICENSE 文件，README / setup.py 同步标注 MIT
- ✨ **依赖策略**：纯 Python 包走 pip，需编译的原生包走预编译产物（libs/ + setup_libs.sh）
- 📊 **统计数据更新**：CNB 仓库 1323 个文件 / 312 MB

### v4.2 (2026-05-19 22:23) — UDS主通道（Plugin直连）+ ZMQ+mmap DAG + 统一深度管线 + 用户位置注入
